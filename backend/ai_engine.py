...
- Even if available Cash is less than {5000 if config.SELECTED_EXCHANGE == "UPBIT" else 10}, do NOT force the decision to 'HOLD' solely due to lack of cash. The system will handle cash constraints and execution.
- Analyze the market indicators objectively. If you detect a valid buy entry, recommend 'BUY' and state the technical reasons in the 'reason' field (e.g. based on RSI, MACD, etc.).
